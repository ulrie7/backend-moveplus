...
module.exports = class RouteServices extends CoreServices {

  constructor() {
    super();
    this.Route = require("../../admin/models/trajet.model");
  ...
  instanceAlreadyExist = async (payload, session = null) => {
    return await this.SessionManager.executeQueryHookWithSession(this.Route.findOne({
      // your condition
      name: payload.name,
    }), session)
  };
  ...
  findOrCreate = async (payload, session = null) => {
    const routeExist = await this.instanceAlreadyExist(payload, session)
  ...
  create = async (payload, session = null) => {
    ...
    const route = new this.Route(schema);
    const save = await route.save(options);
  ...
  update = async (query, payload, session = null) => {
    ...
    const route = await this.SessionManager.executeQueryHookWithSession(this.Route.findOne(query), session);

    if (!route) throw new this.NotFoundError(this.ERROR_MESSAGES.CAN_NOT_FIND('this route'))
    ...
    const data = await this.Route.findOneAndUpdate({
        _id: route._id
      },
      schema, options
    );
  ...
  delete = async (query, session = null) => {
    ...
    await route.softDelete(undefined, session);
  ...
  findById = async (id, session = null) => {
    const route = await this.SessionManager.executeQueryHookWithSession(this.Route.findOne({
      _id: id
    }), session)

    if (!route) throw new this.NotFoundError(this.ERROR_MESSAGES.CAN_NOT_FIND('this route'))

    return await this.RouteResources.ref(route)
  };
  /**
   * @getList
   */
  getList = async (querySchema, match = {
    ...
    const data = []
    const routeFindData = await this.SessionManager.executeQueryHookWithSession(this.Route.find(querySchema).populate({
      path: "vehicle_id",
      match: matchVehicle_id
    }), session)

    for (const item of routeFindData) {
      if (item
        //&&item.vehicle_id
      ) {
  ...
  findOne = async (querySchema, session = null) => {
    ...
    const routeFindOneData = await this.SessionManager.executeQueryHookWithSession(this.Route.findOne(querySchema).populate({
      path: "vehicle_id",
    }), session)

    if (routeFindOneData) {
      data = await this.RouteResources.collection(routeFindOneData)
    }
