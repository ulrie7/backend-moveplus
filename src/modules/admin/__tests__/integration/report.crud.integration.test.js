jest.mock('../../../../config/directory/index', () => {});
const mongoose = require('mongoose');
const { setupTestDB, teardownTestDB } = require('../../../../../tests/setup/db.handler');

const { validReport, reportWithAllFields } = require('../fixtures/report.fixtures');

describe('Report CRUD Integration', () => {
  let mongoServer;
  let app;
  let request;


  let createdId;

  beforeAll(async () => {
    ({ mongoServer, app, request } = await setupTestDB(() => require('../../../../app')));


  });

  afterAll(async () => {
    await teardownTestDB(mongoServer);
  });

  describe('POST /api/v1/report', () => {
    it('should create a new report (200)', async () => {
      const res = await request
        .post('/api/v1/report')
        .send(reportWithAllFields);

      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data).toBeDefined();
      createdId = res.body.data._id;
    });


  });

  describe('GET /api/v1/report', () => {
    it('should return list of reports (public)', async () => {
      const res = await request
        .get('/api/v1/report')
        .query({ page: 1, perPage: 10 });

      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data).toBeDefined();
    });
  });

  describe('GET /api/v1/report/:id', () => {
    it('should return a report by id', async () => {
      expect(createdId).toBeDefined();

      const res = await request
        .get(`/api/v1/report/${createdId}`);

      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data).toBeDefined();
    });

    it('should return 401/404 for non-existent id', async () => {
      const fakeId = new mongoose.Types.ObjectId();
      const res = await request
        .get(`/api/v1/report/${fakeId}`);

      expect([401, 404]).toContain(res.status);
    });

  });

  describe('PUT /api/v1/report/:id', () => {
    it('should update a report', async () => {
      expect(createdId).toBeDefined();

      const res = await request
        .put(`/api/v1/report/${createdId}`)
        .send({
          report_type: 'Updated Report_type',
          date_generation: '2025-12-31',
        });

      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

  });

  describe('DELETE /api/v1/report/:id', () => {
    it('should delete a report (soft delete)', async () => {
      expect(createdId).toBeDefined();

      const res = await request
        .delete(`/api/v1/report/${createdId}`);

      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

  });
});